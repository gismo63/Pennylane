...
def optimize_circuit(params,hamiltonian):
    ...
    opt = qml.AdamOptimizer() # Our optimizer!
    max_iterations = 1000 # Maximum number of calls to the optimizer 
    conv_tol = 1e-09 # Convergence threshold to stop our optimization procedure
    params = np.array(params)

    for n in range(max_iterations):
        [params, grad], prev_cost = opt.step_and_cost(circuit, params, hamiltonian)
        #print(params[0])
        ...
        if n % 10 == 0:
            print(f"Step = {n},  Cost function = {cost:.8f} ")
        if conv <= conv_tol:
            break
