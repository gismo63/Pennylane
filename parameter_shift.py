...
def parameter_shift(weights):
    ...
    gradients = np.zeros(shape = weights.shape)

    for i in range(weights.shape[0]):
        for j in range(weights.shape[1]):
            weights[i][j] += np.pi/2
            forward = circuit(weights)
            weights[i][j] -= np.pi
            backward = circuit(weights)
...
for i, (input_, expected_output) in enumerate(test_cases):
    ...
    output = run(input_)
    print(output)
    insS = np.array(json.loads(input_), requires_grad = True)
    outS = qml.gradients.param_shift(circuit)(insS).round(3).tolist()
    print(outS)
    """
    try:
        output = run(input_)
